...
const medId = sessionStorage.getItem('medicationId')
...
const postMedication = async () => {
    ...
    if (medName && (medicationTimes != "default") && (startDate != "Invalid Date") && (endDate != "Invalid Date")) {
        if(medId){
        const medData = await fetch(`/medication/${medId}`, {
            method: 'PUT',
            body: JSON.stringify({
                name: medName,
...
initialize = async () => {
    ...
    if(medData.custom_schedule){
        const confirm = window.confirm('Would you like to keep using a custom schedule?')
        if(confirm){
            customScheduleView()
            sunCheck.checked = medData.custom_schedule.includes('0')
            monCheck.checked = medData.custom_schedule.includes('1')
            tueCheck.checked = medData.custom_schedule.includes('2')
            wedCheck.checked = medData.custom_schedule.includes('3')
            thuCheck.checked = medData.custom_schedule.includes('4')
            friCheck.checked = medData.custom_schedule.includes('5')
            satCheck.checked = medData.custom_schedule.includes('6')
        }
    }
}
